...
import serviceAccountkey from './recat-js-blog-firebase-adminsdk-xuffz-b249e507e4.json'  assert { type: 'json' }
...
app.post('/signup',(req,res)=>{
    ...
    if(fullname.length < 3){
        res.status(403).json({'error':'Fullname must be atleast 3 character'})
    }
    if(!email.length){
    ...
    if(!passwordRegex.test(password)){
        res.status(403).json({'error':'Password should be 6 to 20 Characters long with a numeric and 1 uppercase'})
    }
      ...
      User.findOne({'personal_info.email':email}).then((user)=>{
        ...
        if(!user.google_auth){
            bcrypt.compare(password,user.personal_info.password,(err,result)=>{
                if(err){
                    res.status(403).json({'error':'error occured while signin please try again'})
                }
          ...
          return res.status(403).json({'error': 'Account was created with google . Try login with Google.'})
        }
    ...
    app.post('/google-auth',async (req,res)=>{
        ...
        .verifyIdToken(access_token).then(async (decodedUser)=>{
            let {email,name,picture} = decodedUser;
             ...
             if(user){
                if(!user.google_auth){
                 return res.status(403).json({'error' : 'This email was signed without google. please login with password to access the account'})
    ...
    app.post('/change-password',verifyJWT,(req,res)=>{
        ...
        if(!passwordRegex.test(currentPassword) || !passwordRegex.test(newPassword)){
            return res.status(500).json({error:"Password should be 6 to 20 Characters long with a numeric and 1 uppercase" })
         }

         User.findOne({_id:req.user}).then(user=>{
            if(user.google_auth){
                return res.status(403).json({error:"You Can't change Account's password because you logged in throught google"})
            }
            ...
            bcrypt.compare(currentPassword,user.personal_info.password,(err,result)=>{
                ...
                bcrypt.hash(newPassword,10,(err,hashed_password)=>{
                    User.findOneAndUpdate({_id:req.user},{"personal_info.password":hashed_password}).then((u)=>{
                        ...
                        return res.status(500).json({error:"Some error Occured while changing Password plz try again later"})
                    })  
                })
            })
         }).catch(err=>{
 ...
 app.post('/search-blogs-count',(req,res)=>{
    ...
    if(tag){
     ...
     Blog.countDocuments(findQuery).then(count=>{
        return res.status(200).json({toatalDocs:count})
     })
     .catch(err=>{
        return res.status(500).json({error:err.message})
     })
   ...
   app.post('/update-profile-img',verifyJWT,(req,res)=>{
    ...
    User.findOneAndUpdate({_id:req.user},{"personal_info.profile_img":url}).then((response)=>{
          ...
          for(let i = 0; i<socialLinksArr; i++){
            if(social_links[socialLinksArr[i]].length){
                ...
                if(!hostname.includes(`${socialLinksArr[i]}`.com) && socialLinksArr[i] != 'website' ){
                    return res.status(403).json({error:`${socialLinksArr[1]} link is invalid. You must enter a full link `})
                }
            }
          }
   ...
   app.post('/create-blog',verifyJWT,(req,res)=>{
    ...
    if(!draft){
        if(!des.length || des.length > 200) {
            return res.status(403).json({error : 'You must provide description undur 200 characters '})
        }
        ...
        if(!tags.length || tags.leength > 10){
          return res.status(403).json({error:'provide tags in order to publsh the blog maximum 10'})
        }
    ...
    blog.save().then(blog=>{
        ...
        let incremetnVal = draft ? 0 : 1;

        User.findOneAndUpdate({_id:AuthorId}, { $inc:{"account_info.total_posts" : incremetnVal},
         ...
         $push:{'blogs': blog._id} }).then((user)=>{
            ...
            return res,staus(500).json({error:'Failed to upadate total post number'})
         })
    }).catch(err=>{
        return res.status(500).json({error:err.message + 'this is that'})
    })
  }
   ...
   app.post("/add-comment",verifyJWT,(req,res)=>{
     ...
     if(replying_to){ 
       ...
       new Comment(commentObj).save().then(async commentFile =>{
        ...
        "activity.total_parent_comments": replying_to ? 0 : 1}}).then(blog=>{
        ...
        if(replying_to){ 
            ...
            .then(replyToComment => {notificationObj.notification_for = replyToComment.commented_by})
   ...
   const deleteComment = (_id)=>{
        Comment.findOneAndDelete({_id}).then(comment=>{
            ...
            Notification.findOneAndDelete({comment:_id}).then(notification=>console.log(notification))

            Notification.findOneAndUpdate({reply:_id},{$unset:{reply: 1}}).then(notification=>console.log(notification))

            Blog.findOneAndUpdate({_id:comment.blog_id},{$pull:{comments:_id},$inc:{"activity.total_comments":-1,
            "activity.total_parent_comments":comment.parent ? 0 : -1}}).then(blog =>{
    ...
    .then(comment=>{
        ...
        if(user_id == comment.commented_by || user_id == comment.blog_author){
           ...
           return res.status(200).json({status:'Done'}) 
            ...
            return res.status(403).json({error : 'You dont have access to delete the comment'})
        }
    })
